...
preclinical_pie.ggplot <- .preclin_pie(Biobase::fData(preclinical.eset), y.c = "category",
...
```{r}
irt_liver.gg <- ProMetIS:::.irt_plot(irt_liver.mn)
irt_plasma.gg <- ProMetIS:::.irt_plot(irt_plasma.mn, "plasma")
cv_liver.gg <- ProMetIS:::.cv_ggplot(cv_liver.df)
...
for (set.c in grep("(c18hyper|hilic)", names(metabo.mset), value = TRUE)) {
  ...
  detected_standards.vc <- switch(set.c,
                                  metabolomics_liver_c18hypersil_pos = {
                                    c("Glucose 13C [ExS]", "Aspartate 15N [ExS]", "2-Aminoanthracene [ExS]", "Amiloride [ExS]", "Imipramine [ExS]", "Prednisone [ExS]")
                                  },
                                  metabolomics_liver_hilic_neg = {
                                    c("Glucose 13C [ExS]", "Amiloride [ExS]")
                                  },
                                  metabolomics_plasma_c18hypersil_pos = {
                                    c("Dimetridazole [IS]", "Glucose 13C [ExS]", "Imipramine [ExS]", "Atropine [ExS]", "Prednisone [ExS]", "Dihydrostreptomycin [ExS]")
                                  },
                                  metabolomics_plasma_hilic_neg = {
                                    c("MCPA [IS]", "AMPA [IS]", "Glucose 13C [ExS]", "Dinoseb [IS]")
                                  })
  tlog2exprs.df <- tlog2exprs.df[, detected_standards.vc, drop = FALSE]
  ...
  standards.ggplot <- ggplot2::ggplot(ggplot.df,
    ...
    ggplot2::geom_ribbon(ggplot2::aes(x = injectionOrder, ymin = lcl, ymax = ucl, group = compound, fill = compound, linetype = "dashed"), alpha = 0.1) +
...
knitr::kable(round(quality.mn))
```

```{r}
quality.df <- as.data.frame(t(quality.mn)[, c("correl_inj_test",
                                              "correl_inj_pca",
                                              "pool_spread_pca",
                                              "poolCV_0.3",
                                              "ICCpool")])
colnames(quality.df) <- c("Drift Spearman", "Drift PCA", "QC spread", "QC CV", "QC ICC")
...
quality_save.mn <- t(as.matrix(quality.df[, -1]))
quality_save.mn <- rbind(quality_save.mn,
                         mean = apply(quality_save.mn, 2, mean),
                         min = apply(quality_save.mn, 2, min),
                         max = apply(quality_save.mn, 2, max))

quality_save.mn <- round(quality_save.mn)

knitr::kable(quality_save.mn)

write.table(quality_save.mn,
            col.names = NA,
...
quality.ggplot <- GGally::ggparcoord(quality.df, columns = c(3, 2, 4:6),
                                     groupColumn = 1,
                                     scale = "globalminmax") +
